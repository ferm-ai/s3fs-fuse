...
# Extract the .deb file to your current directory
    ...
    docker cp s3fs-temp:/s3fs-fuse/s3fs-fuse_1.97-1-agentwork_amd64.deb .
    docker rm s3fs-temp

    # B) Or just use a mount:
    docker run --platform linux/amd64 --rm -v $(pwd):/host s3fs-build cp '/s3fs-fuse/s3fs-fuse_1.97-1-agentwork_amd64.deb' /host/
...
On newer Ubuntu do: (.deb dependencies are currently for 24.04)
    ...
    apt update
    apt install ./s3fs-fuse_1.97-1-agentwork_amd64.deb
...
### Test .deb package on Ubuntu
    ...
    docker run --rm -it --platform linux/amd64 --mount type=bind,src=.,dst=/mnt/ ubuntu
    apt update
    cd /mnt
    apt install ./s3fs-fuse_1.97-1-agentwork_amd64.deb
